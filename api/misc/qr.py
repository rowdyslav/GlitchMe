        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(image_url) as response:
                ...
                if not content_type.startswith(
                    "image/jpeg"
                ) and not content_type.startswith("image/png"):
...
async def gen(
    ...
    try:
        ...
        logo_image_stream = (
            await download_image_from_url(image_url) if image_url else None
        )
        padded_image = process_optional_image(
            logo_image_stream, image_padding, image_round
        )
...
def process_optional_image(
    ...
    if image_stream:
        ...
        logo_image = Image.alpha_composite(
            Image.new("RGBA", logo_image.size, (255, 255, 255, 255)), logo_image
        )
